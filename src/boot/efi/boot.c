...
static BOOLEAN password_read(CHAR8 **line_out) {
        CHAR8 *line;
        UINTN size;
        UINTN len;
        BOOLEAN exit;
...
static UINTN config_defaults_load_from_file(Config *config, CHAR8 *content) {
        ...
        while ((line = line_get_key_value(content, (CHAR8 *)" \t", &pos, &key, &value))) {
                ...
                if (strcmpa((CHAR8 *)"password", key) == 0) {
...
static UINTN config_entry_add_from_file(
                Config *config,
                EFI_HANDLE *device,
                EFI_FILE *root_dir,
                const CHAR16 *path,
...
static void config_entry_add_linux(
        ...
        for (;;) {
                ...
                _cleanup_freepool_ CHAR16 *os_pretty_name = NULL, *os_image_id = NULL, *os_name = NULL, *os_id = NULL, *os_id_like = NULL;
                ...
                while ((line = line_get_key_value(content, (CHAR8 *)"=", &pos, &key, &value))) {
                        ...
                        if (strcmpa((const CHAR8 *)"ID_LIKE", key) == 0) {
                                FreePool(os_id_like);
