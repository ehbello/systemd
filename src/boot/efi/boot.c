...
static bool password_read(CHAR8 **line_out) {
        CHAR8 *line;
        UINTN size;
        UINTN len;
        BOOLEAN exit;
...
static UINTN config_defaults_load_from_file(Config *config, char *content) {
        char *line;
        UINTN pos = 0;
        char *key, *value;
        EFI_STATUS err;
...
static UNINT config_entry_add_type1(
                Config *config,
                EFI_HANDLE *device,
                EFI_FILE *root_dir,
                const char16_t *path,
...
static void config_load_defaults(Config *config, EFI_HANDLE *device, EFI_FILE *root_dir, CHAR16 *loaded_image_path, VOID *image_base) {
        ...
        *config = (Config) {
        ...
        if (!EFI_ERROR(err) && len > 0) {
                ...
                err = file_read(root_dir, L"\\loader\\loader.conf", 0, 0, &content, &len);
        }
